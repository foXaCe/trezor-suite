...
import { Button, CollapsibleBox, Card as TrezorCard, variables } from '@trezor/components';
            ...
            ```javascript
                TrezorConnect.init({
                    manifest: {
                        ...
                        // svg or png icon
                        appIcon: 'http://your.application.com/icon.svg',
...
<SectionCard>
    ...
    <Tabs items={['Suite desktop popup', 'Suite Web popup', 'Auto mode selection']}>
        <Tabs.Tab>
            {/* todo: add image */}
            <div> Suite desktop is focused and popup is presented to user</div>
        </Tabs.Tab>
        <Tabs.Tab>
...
<SectionCard>
    ...
    <SdkContainer>
        ...
        <ExamplesAside>
            <ExampleHeading>Examples:</ExampleHeading>
            - [Simple example](https://github.com/trezor/trezor-suite/tree/develop/packages/connect-examples/webextension)
        </ExamplesAside>
    </SdkContainer>
        ...
        <CollapsibleBox heading="Quick start manual" margin={{ top: spacings.md }}>
            <Steps>
                ...
                The popup flow no longer uses `connect-script` or inline iframe injection. Instead, the service worker opens
                the Suite Web popup and the popup communicates back using the `externally_connectable` API. Make sure your
                extension allows the Suite Web origin you use to send messages back:
