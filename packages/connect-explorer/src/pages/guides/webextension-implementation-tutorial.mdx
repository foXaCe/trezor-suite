...
import { TutorialSectionCard as SectionCard } from '../../components/IndexPageComponents';
...
<SectionCard>
    ...
    The popup flow no longer relies on `connect-script` or iframe injection. The Suite Web popup communicates with the
    service worker via `externally_connectable`.
