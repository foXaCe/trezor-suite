...
export const LiveFiatGraph = ({ account, isLive, isGraphLoading = false }: LiveFiatGraphProps) => {
    ...
    const displayedResolutionEntry = useSelector(state =>
        coingeckoId
            ? selectGraphFiatResolutionEntry(
                  state,
                  coingeckoId,
                  historicalFiatCurrencyCode,
                  displayedResolution,
              )
            : null,
    );
    const displayedPriceHistory = useMemo(
        () => displayedResolutionEntry?.points ?? [],
        [displayedResolutionEntry],
    );
    ...
    const markers = useMemo((): LivelineMarker[] => {
        if (isLive || isTransitioning) return [];

        const accountData = graphData.find(d => accountGraphDataFilterFn(d, account));
        if (!accountData) return [];

        // Align marker positions with the visible line by snapping them to the
        // first price-history point at or after the balance-step time.
        const snapToPrice = (time: number): number => {
            const idx = displayedPriceHistory.findIndex(p => p.time >= time);

            return idx >= 0 ? displayedPriceHistory[idx].time : time;
        };
        ...
        const rawEntries = accountData.data
            ...
            .map(entry => {
                ...
                return { time: snapToPrice(entry.time), sent, received };
            })
