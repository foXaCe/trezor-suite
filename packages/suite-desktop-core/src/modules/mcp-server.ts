...
export const init: ModuleInit = ({ mainWindowProxy, store }) => {
    ...
    const startServer = async (port: number) => {
        ...
        // Global middleware: Bearer token authentication
        httpServer.use([
        ...
        httpServer.post('/mcp', [
            ...
            async (request: RequestWithParams, response: Response) => {
                ...
                const requestSessionId = request.headers['mcp-session-id'] as string | undefined;
                ...
                const jsonRpcRequest = request.body as {
                    id?: string | number;
                    method: string;
                    params?: Record<string, unknown>;
                };
                ...
                const jsonRpcResponse = await handleJsonRpcRequest(jsonRpcRequest, sendPopupCall);
