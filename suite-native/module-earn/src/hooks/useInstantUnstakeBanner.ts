...
export const useInstantUnstakeBanner = (
    ...
    useEffect(() => {
        ...
        if (transfer && getInstantStakeType(transfer, descriptor, symbol) === 'unstake') {
            const changedTxid = txs.find(tx => tx.internalTransfers.includes(transfer))?.txid;

            if (changedTxid && !seenTxIds.current.has(changedTxid)) {
                seenTxIds.current.add(changedTxid);
