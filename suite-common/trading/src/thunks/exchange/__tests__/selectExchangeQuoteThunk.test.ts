...
import { type RefetchQuotesState, initialState } from '../../../reducers/tradingCommonReducer';
...
describe('selectExchangeQuoteThunk', () => {
    ...
    const getMocks = (
        ...
        refetchQuotesOverride?: Partial<RefetchQuotesState>,
    ) => {
    ...
    it('should successfully select quote', async () => {
        ...
        expect(store.getState().wallet.trading.refetchQuotes.status).toBe('stopped');
